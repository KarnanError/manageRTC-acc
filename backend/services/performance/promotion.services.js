...
const validateCreate = async (collections, promotion, existingPromotionId = null) => {
  ...
  // Validate that employee exists and fetch current designation FIRST
  let employee;
  try {
    ...
    console.error("[PromotionService] Error finding employee:", error);
    return "Error validating employee: " + error.message;
  }
  ...
  try {
    ...
    console.error("[PromotionService] Error in lifecycle validation:", error);
    return "Error validating employee lifecycle: " + error.message;
  }
  ...
  try {
    ...
    console.error("[PromotionService] Error validating designation/date:", error);
  ...
  try {
    ...
    console.error("[PromotionService] Error checking overlapping promotions:", error);
    return "Validation error: " + error.message;
  }
...
export const getAllPromotions = async (companyId, filters = {}) => {
  try {
    ...
    console.log("[PromotionService] Query:", JSON.stringify(query));

    // Simple aggregation that returns raw data with manual lookups
    const rawPromotions = await collections.promotions.find(query)
      .sort({ promotionDate: -1, createdAt: -1 })
      .toArray();

    console.log(`[PromotionService] Found ${rawPromotions.length} raw promotions`);

    // Manually resolve references for each promotion
    const promotions = await Promise.all(rawPromotions.map(async (promo) => {
      console.log(`[PromotionService] Processing promotion ${promo._id}`);

      // Lookup employee
      let employee = null;
      try {
        if (promo.employeeId) {
          const empId = ObjectId.isValid(promo.employeeId) ? new ObjectId(promo.employeeId) : null;
          console.log(`[PromotionService] Looking up employee with _id: ${empId}`);
          employee = await collections.employees.findOne({ _id: empId });
          console.log(`[PromotionService] Employee found: ${employee ? employee.firstName + ' ' + employee.lastName : 'NOT FOUND'}`);
        }
      } catch (err) {
        console.error("[PromotionService] Error looking up employee:", err.message);
      }

      // Lookup promotionFrom department
      let fromDept = null;
      try {
        if (promo.promotionFrom?.departmentId) {
          const deptId = ObjectId.isValid(promo.promotionFrom.departmentId) ? new ObjectId(promo.promotionFrom.departmentId) : null;
          console.log(`[PromotionService] Looking up FROM department with _id: ${deptId}`);
          fromDept = await collections.departments.findOne({ _id: deptId });
          console.log(`[PromotionService] FROM Department found: ${fromDept ? fromDept.department || fromDept.name : 'NOT FOUND'}`);
        }
      } catch (err) {
        console.error("[PromotionService] Error looking up from department:", err.message);
      }

      // Lookup promotionFrom designation
      let fromDesig = null;
      try {
        if (promo.promotionFrom?.designationId) {
          const desigId = ObjectId.isValid(promo.promotionFrom.designationId) ? new ObjectId(promo.promotionFrom.designationId) : null;
          console.log(`[PromotionService] Looking up FROM designation with _id: ${desigId}`);
          fromDesig = await collections.designations.findOne({ _id: desigId });
          console.log(`[PromotionService] FROM Designation found: ${fromDesig ? fromDesig.designation || fromDesig.name : 'NOT FOUND'}`);
        }
      } catch (err) {
        console.error("[PromotionService] Error looking up from designation:", err.message);
      }

      // Lookup promotionTo department
      let toDept = null;
      try {
        if (promo.promotionTo?.departmentId) {
          const deptId = ObjectId.isValid(promo.promotionTo.departmentId) ? new ObjectId(promo.promotionTo.departmentId) : null;
          console.log(`[PromotionService] Looking up TO department with _id: ${deptId}`);
          toDept = await collections.departments.findOne({ _id: deptId });
          console.log(`[PromotionService] TO Department found: ${toDept ? toDept.department || toDept.name : 'NOT FOUND'}`);
        }
      } catch (err) {
        console.error("[PromotionService] Error looking up to department:", err.message);
      }

      // Lookup promotionTo designation
      let toDesig = null;
      try {
        if (promo.promotionTo?.designationId) {
          const desigId = ObjectId.isValid(promo.promotionTo.designationId) ? new ObjectId(promo.promotionTo.designationId) : null;
          console.log(`[PromotionService] Looking up TO designation with _id: ${desigId}`);
          toDesig = await collections.designations.findOne({ _id: desigId });
          console.log(`[PromotionService] TO Designation found: ${toDesig ? toDesig.designation || toDesig.name : 'NOT FOUND'}`);
        }
      } catch (err) {
        console.error("[PromotionService] Error looking up to designation:", err.message);
      }

      // Build response object matching frontend expectations
      return {
        _id: promo._id,
        promotionId: promo._id.toString(), // Add promotionId field
        companyId: promo.companyId,
        employeeId: promo.employeeId,
        employeeName: employee ? `${employee.firstName || ''} ${employee.lastName || ''}`.trim() : promo.employeeName || 'Unknown',
        promotionFrom: {
          departmentId: promo.promotionFrom?.departmentId || '',
          department: fromDept?.department || fromDept?.name || fromDept?.departmentName || promo.promotionFrom?.department || '',
          designationId: promo.promotionFrom?.designationId || '',
          designation: fromDesig?.designation || fromDesig?.name || promo.promotionFrom?.designation || ''
        },
        promotionTo: {
          departmentId: promo.promotionTo?.departmentId || '',
          department: toDept?.department || toDept?.name || toDept?.departmentName || '',
          designationId: promo.promotionTo?.designationId || '',
          designation: toDesig?.designation || toDesig?.name || ''
        },
        promotionDate: promo.promotionDate,
        promotionType: promo.promotionType,
        salaryChange: promo.salaryChange,
        reason: promo.reason,
        notes: promo.notes,
        status: promo.status,
    ...
    console.log(`[PromotionService] Found ${promotions.length} promotions`);
    if (promotions.length > 0) {
      console.log("[PromotionService] Sample promotion:", JSON.stringify(promotions[0], null, 2));
    }
